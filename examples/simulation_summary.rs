...
fn main() {
    ...
    // log
    println!("{}:{}:Updated:{messages_sent}:{messages_received}:{bytes_sent}:{bytes_received}:{event_counter}:{process_counter}", args.num_participants, args.seed);
}
