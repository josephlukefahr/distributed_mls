...
fn handle_event(
    ...
    match event {
        ...
        Event::Broadcast(i, message) => {
            ...
            for j in 0..agents.len() as u16 {
                if j != i {
                    events.push(Event::Deliver(message.clone(), j));
                }
            }
            // log
            *messages_sent += 1;
        ...
        Event::Deliver(message, j) => {
            ...
            *bytes_received += message.mls_encoded_len();
            println!("{event_counter}:Deliver({:x},{j}):{messages_sent}:{messages_received}:{bytes_sent}:{bytes_received}:{process_counter}", md5::compute(&message.to_bytes().unwrap()));
        }
        Event::Process(j) => {
            // process message for participant j
            let message_in = queues[j as usize].dequeue().unwrap();
